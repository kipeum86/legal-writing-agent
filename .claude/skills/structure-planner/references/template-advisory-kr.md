...
- For Korean legal opinions, `configured supplemental reference` controls where it is more specific than this template
...
2. 정보 블록 `[B]`
   ...
   - 발신: `[작성 주체 / KP Legal Orchestrator]`
   - 일자: `[날짜]`
   - 제목: `[검토 제목]`
...
10. `8. 종결 문구 및 서명 블록` `[B]`
    - 한국 실무 문서 관행상 `이상입니다.` 또는 `끝.`을 사용 가능
    - 서명 블록 예시:
...
- 법령 블록과 판례 인용은 `configured supplemental reference`의 형식을 우선 적용한다
