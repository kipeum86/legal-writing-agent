...
1. Select convention set: Language + Jurisdiction → base style guide; Document type → custom template override from `/library/templates/` when available, otherwise built-in template from `.claude/skills/structure-planner/references/`; House style overlay. **한국어 법률의견서인 경우 `configured supplemental reference`를 반드시 읽어 적용.**
...
### Local Korean Advisory Formatting Profile

**한국어 법률의견서(법률검토의견, 클라이언트 메모 포함)를 작성할 때는 반드시 `configured supplemental reference`를 읽고 그 규칙을 따를 것.** 이 가이드는 실제 법무법인 작성 의견서 3건에서 추출한 스타일 규칙을 체계화한 것으로, 다음을 포함합니다:
