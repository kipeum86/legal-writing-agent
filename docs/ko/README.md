...
> **이 도구는 법률문서 작성 보조용이며, 법률 자문을 제공하지 않습니다.** 결과물은 AI가 생성하며, 자체 검토에도 불구하고 오류가 포함될 수 있습니다. 모든 문서는 법률 전문가의 검토 후 사용해야 합니다. **[면책사항](./DISCLAIMER.md)** · **[Full Disclaimer](../en/DISCLAIMER.md)**
...
- **수정**: 기존 법률문서의 변경 추적 결과물을 포함한 수정 (R1~R7 파이프라인)
...
R1  문서 분석 ──► R2  수정 범위 ──► R3  규범 검토 ──► R4  수정 실행
                                                          │
R7  파일 저장 ◄── R6  출력 (변경 추적 / redline) ◄── R5  일관성 검토 ◄┘
```
...
`/library/` 폴더에는 세 가지 유형의 재사용 가능한 자산이 포함됩니다. **사용자가 직접 관리**하며 기본적으로 **gitignore** 처리되어 버전 관리에서 제외되기 쉽고, 인제스트 메타데이터 파일인 `library/source-registry.json`도 기본적으로 저장소 밖에 머물도록 무시됩니다. 다만 그 내용이 프롬프트에 포함되어 전송되면, 해당 부분은 Claude Code / Anthropic 데이터 처리 설정의 적용을 받습니다.
...
1. `library/inbox/`에 파일(PDF, DOCX 등)을 넣습니다
2. 에이전트에게 알려줍니다: `/ingest` 또는 "파일 넣었어"
3. 에이전트가 자동으로 처리합니다:
   - Markdown으로 변환
   - 소스 등급 분류 (A/B/C)
   - 메타데이터(frontmatter) 생성
   - `library/grade-{a,b,c}/` 적절한 폴더에 배치
   - 인덱스 업데이트

> **참고:** 파일을 넣는 것만으로는 자동 처리되지 않습니다.
> `/ingest`를 실행하거나 에이전트에게 알려줘야(예: "inbox에 파일 넣었어") 파싱이 시작됩니다.
...
| [general-legal-research](https://github.com/lowtidebuild/general-legal-research) | 김재식 | 법률 리서치 스페셜리스트 |
| [PIPA-expert](https://github.com/lowtidebuild/PIPA-expert) | 정보호 | 개인정보 스페셜리스트 |
| [GDPR-expert](https://github.com/lowtidebuild/GDPR-expert) | 김덕배 | 데이터 보호 스페셜리스트 |
| [contract-review-agent](https://github.com/lowtidebuild/contract-review-agent) | 고덕수 | 계약 검토 스페셜리스트 |
