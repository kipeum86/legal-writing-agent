...
if grep -nE "(supplemental-reference|naming-transition|codex-quality-audit|legal-writing-agent-design|source-registry|draft\.md|change-map\.json|outline\.json)" .gitignore; then
    echo "FAIL: private filename detected in .gitignore"
    fail=1
else
    echo "ok: .gitignore carries no individual private filenames"
fi

# 2. Old style-guide path is gone
if rg -n "configured supplemental reference" -g '!docs/plans/*' -g '!docs/_private/*' -g '!tools/security/checks/gitignore-hygiene.sh'; then
    echo "FAIL: legacy style-guide path still referenced"
    fail=1
else
    echo "ok: no residual references to configured supplemental reference"
fi
